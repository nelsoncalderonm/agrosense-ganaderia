...
export default function Drawer({ open, finca, user, onClose, onNavigate, onSetRole }: Props) {
  if (!open) return null;
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', zIndex: 40 }} />
      <div className="animate-slide-up" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#EEF2EC', borderRadius: '16px 16px 0 0', zIndex: 50, maxHeight: '85%', overflowY: 'auto', paddingBottom: 24 }}>
        {/* Finca header */}
        <div style={{ padding: '20px 20px 14px', borderBottom: '1px solid #E1E8DD' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 16 }}>{finca.nombre}</div>
              <div style={{ fontFamily: 'var(--font-jetbrains)', fontSize: 11, color: '#6E8A6E', marginTop: 2 }}>{finca.ubic} · {finca.animales} animales</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '8px 8px 0' }}>
          {MENU_ITEMS.map(item => (
            <button key={item.key} onClick={() => { onNavigate(item.key); onClose(); }} style={{
              ...
              padding: '12px 12px', borderRadius: 8, background: 'none', border: 'none',
              cursor: 'pointer', textAlign: 'left', color: '#1A2B1A',
            }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: '#fff', border: '1px solid #E1E8DD', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {item.special === 'cow' ? (
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1A2B1A" strokeWidth="1.8" strokeLinecap="round">
                    <ellipse cx="12" cy="14" rx="7" ry="5"/>
                    <path d="M8 9 Q7 6 5 5M16 9 Q17 6 19 5"/>
                    <circle cx="10" cy="16" r="1" fill="#1A2B1A" stroke="none"/>
                    <circle cx="14" cy="16" r="1" fill="#1A2B1A" stroke="none"/>
                  </svg>
                ) : (
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1A2B1A" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d={item.icon || ''}/>
                  </svg>
                )}
              </div>
        ...
        <div style={{ margin: '12px 12px 0', background: '#fff', border: '1px solid #E1E8DD', borderRadius: 8, overflow: 'hidden' }}>
          ...
          {USERS.map(u => (
            <button key={u.roleKey} onClick={() => { onSetRole(u.roleKey); onClose(); }} style={{
